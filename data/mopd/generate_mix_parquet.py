"""
Generate mixed (mobile + desktop) MOPD reinforcement learning Parquet files.

Usage:
    python generate_mix_parquet.py \
        --mobile_dir /path/to/mobile/data \
        --desktop_dir /path/to/desktop/data \
        --output_dir ./output
Output:
    <output_dir>/mix_mopd_train.parquet
    <output_dir>/mix_mopd_test.parquet
"""
...
def build_ground_truth_mobile(thought, action, plan):
    plan_json = json.dumps(plan, ensure_ascii=False)
    return f"<think>\n{thought}\n</think>\nThought: {action}\nAction: {action}\n<tool_call>\n{plan_json}\n</tool_call>"


def build_ground_truth_desktop(thought, action, plan):
    plan_json = json.dumps(plan, ensure_ascii=False)
    return f"<think>\n{thought}\n</think>\nAction: {action}\n<tool_call>\n{plan_json}\n</tool_call>"
...
def process_mobile_task(task_json_path, traj_dir):
    ...
    for step_info in data_steps:
        ...
        bbox = step_info.get("bbox") or None
        bbox2 = step_info.get("bbox2") or None

        rows.append({
            "data_source": "mobile",
            "prompt": prompt,
            "images": images,
            "reward_model": {"ground_truth": build_ground_truth_mobile(thought, action, plan)},
            "ability": "gui_agent",
            "extra_info": {
...
def process_desktop_task(task_json_path, traj_dir):
    ...
    for step_info in data_steps:
        ...
        rows.append({
            ...
            "prompt": prompt,
            "images": images,
            "reward_model": {"ground_truth": build_ground_truth_desktop(thought, action, plan)},
            "ability": "gui_agent",
            "extra_info": {
                "id": f"{episode_id}_step{step_num}",
                "episode_id": episode_id,
...
def main():
    parser = argparse.ArgumentParser(description="Generate mixed MOPD parquet from mobile + desktop data.")
    ...
    parser.add_argument("--test_traj_per_side", type=int, default=TEST_TRAJ_PER_SIDE, help="Number of test trajectories per platform")
    ...
    mobile_traj_dirs = sorted(mobile_traj_rows.keys())
    desktop_traj_dirs = sorted(desktop_traj_rows.keys())
    random.shuffle(mobile_traj_dirs)
    random.shuffle(desktop_traj_dirs)

    mobile_test_dirs = set(mobile_traj_dirs[:args.test_traj_per_side])
    mobile_train_dirs = set(mobile_traj_dirs[args.test_traj_per_side:])
    desktop_test_dirs = set(desktop_traj_dirs[:args.test_traj_per_side])
    desktop_train_dirs = set(desktop_traj_dirs[args.test_traj_per_side:])

    train_rows = []
    test_rows = []

    for d in mobile_train_dirs:
        train_rows.extend(mobile_traj_rows[d])
    for d in desktop_train_dirs:
        train_rows.extend(desktop_traj_rows[d])
    for d in mobile_test_dirs:
        test_rows.extend(mobile_traj_rows[d])
    for d in desktop_test_dirs:
        test_rows.extend(desktop_traj_rows[d])
    ...
    print(f"Mobile  test: {len(mobile_test_dirs)} trajectories ({test_mobile} steps)")
    print(f"Desktop test: {len(desktop_test_dirs)} trajectories ({test_desktop} steps)")
    print(f"Mobile  train: {len(mobile_train_dirs)} trajectories ({train_mobile} steps)")
    print(f"Desktop train: {len(desktop_train_dirs)} trajectories ({train_desktop} steps)")
    print(f"\nTrain total: {len(train_rows)} steps (mobile {train_mobile}, desktop {train_desktop})")
    print(f"Test  total: {len(test_rows)} steps (mobile {test_mobile}, desktop {test_desktop})")
    ...
    train_path = os.path.join(args.output_dir, "mix_mopd_train.parquet")
    ...
    if test_rows:
        ...
        test_path = os.path.join(args.output_dir, "mix_mopd_test.parquet")
