"""
ScreenSpot-V2 evaluation script.
Settings:
...
class ScreenSpotV2Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # bbox is [x, y, w, h] in ScreenSpot-V2, convert to [x1, y1, x2, y2]
            bc = item["bbox"]
...
def main():
    ...
    parser.add_argument("--screenspot-imgs", type=str, required=True,
                        help="Path to ScreenSpot-V2 images directory")
    parser.add_argument("--screenspot-test", type=str, required=True,
                        help="Path to ScreenSpot-V2 test data directory")
    ...
    # Stats
    outcome = {
        "mobile": {"text": {"total": 0, "correct": 0}, "icon": {"total": 0, "correct": 0}},
        "desktop": {"text": {"total": 0, "correct": 0}, "icon": {"total": 0, "correct": 0}},
        "web": {"text": {"total": 0, "correct": 0}, "icon": {"total": 0, "correct": 0}},
    }

    tasks = ["mobile", "desktop", "web"]

    for task in tasks:
        data_path = os.path.join(args.screenspot_test, f"screenspot_{task}_v2.json")
        with open(data_path, 'r') as f:
            screenspot_data = json.load(f)
        if rank == 0:
            print(f"\nLoaded {task}: {len(screenspot_data)} samples")

        dataset = ScreenSpotV2Dataset(screenspot_data, processor, args.screenspot_imgs,
        ...
        pbar = tqdm(dataloader, desc=f"Processing {task}") if rank == 0 else dataloader
        ...
        for batch_data in pbar:
            ...
            try:
                ...
                for idx, output_text in enumerate(output_texts):
                    ...
                    outcome[task][data_type]["total"] += 1
                    if is_correct:
                        outcome[task][data_type]["correct"] += 1
    ...
    if is_distributed:
        ...
        if rank == 0:
            final_outcome = {
                t: {"text": {"total": 0, "correct": 0}, "icon": {"total": 0, "correct": 0}}
                for t in tasks
            }
            for proc_outcome in all_outcomes:
                for task in tasks:
                    for ui_type in ["text", "icon"]:
                        final_outcome[task][ui_type]["total"] += proc_outcome[task][ui_type]["total"]
                        final_outcome[task][ui_type]["correct"] += proc_outcome[task][ui_type]["correct"]
            outcome = final_outcome

    if rank == 0:
        print("=" * 100)
        print("ScreenSpot-V2 Results:")
        overall_correct = 0
        overall_total = 0
        for task in tasks:
            res = outcome[task]
            ...
            print(f"  {task:12s}: {acc*100:.2f}%  text={text_acc*100:.2f}%  icon={icon_acc*100:.2f}%  ({correct}/{total})")
