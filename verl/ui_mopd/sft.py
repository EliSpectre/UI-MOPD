...
from ui_mopd.dataset.dataset_utils import SFTTensorCollator
from ui_mopd.dataset.multiturn_sft_dataset import MultiTurnSFTDataset
...
class SFTTrainer:
    ...
    def fit(self):
        ...
        if global_step > 0:
            print(f"StatefulDataLoader will automatically resume from global step: {global_step}")
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step and self.val_dataloader is not None or (self.test_freq > 0 and is_valid_step):
                    ...
                    if is_logging:
                        ...
                        print(f"step:{global_step} - val/loss:{metric['val/loss']:.6f}")
                        last_valid_metric = metric
                    torch.distributed.barrier()

                if is_last_step or (self.save_freq > 0 and is_save_step):
                    self.ckpt_handler.save_checkpoint(step=global_step, is_final=is_last_step)
...
def create_sft_dataset(data_paths, data_config, processor):
    ...
    dataset = dataset_cls(data_files=data_paths, processor=processor, config=data_config)
    return dataset
