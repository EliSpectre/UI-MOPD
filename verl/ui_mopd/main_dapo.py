...
from ui_mopd.dapo_trainer import RayDAPOTrainer


@hydra.main(config_path="config", config_name="dapo_megatron_trainer", version_base=None)
def main(config):
...
class TaskRunner:
    ...
    def add_ref_policy_worker(self, config, ref_policy_cls):
        ...
        if config.algorithm.use_kl_in_advantage or config.algorithm.use_kl_in_reward or config.actor_rollout_ref.actor.use_kl_loss or config.algorithm.get("use_topk_distillation", False) or config.data.get("use_opsd", False):
    ...
    def run(self, config):
        ...
        reward_fn = load_reward_manager(
            config, tokenizer, num_examine=0, mode="train", **config.reward_model.get("reward_kwargs", {})
        )
        val_reward_fn = load_reward_manager(
            config, tokenizer, num_examine=1, mode="eval", **config.reward_model.get("reward_kwargs", {})
        )
