...
# ---------------------------------------------------------------------------
# Efficient serialization for torch.Tensor objects embedded in non_tensor_batch
# ---------------------------------------------------------------------------
# non_tensor_batch values are np.ndarray(dtype=object) which may hold dicts
# containing torch.Tensor (e.g. multi_modal_inputs with pixel_values).  When
# pickled naively, each tensor goes through torch.Tensor.__reduce_ex__ which is
# slower than pickle5 buffer-protocol on contiguous numpy arrays.  The helpers
# below convert tensors to (tag, numpy_buffer) tuples *before* pickle, so the
# heavy payload uses pickle5 zero-copy into Ray Plasma store.
# ---------------------------------------------------------------------------

_VERL_TENSOR_TAG = "__verl_st__"


def _serialize_obj(obj):
    """Recursively convert torch.Tensor to numpy-backed form for pickle5 zero-copy."""
    if isinstance(obj, torch.Tensor):
        return (_VERL_TENSOR_TAG, serialize_single_tensor(obj))
    if isinstance(obj, dict):
        return {k: _serialize_obj(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        converted = [_serialize_obj(v) for v in obj]
        return type(obj)(converted)
    return obj


def _deserialize_obj(obj):
    """Restore torch.Tensor from numpy-backed serialized form."""
    if isinstance(obj, tuple) and len(obj) == 2 and obj[0] == _VERL_TENSOR_TAG and isinstance(obj[1], tuple):
        # .clone() copies into torch-allocated memory for efficient GPU transfer
        return deserialize_single_tensor(obj[1]).clone()
    if isinstance(obj, dict):
        return {k: _deserialize_obj(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        converted = [_deserialize_obj(v) for v in obj]
        return type(obj)(converted)
    return obj


def _serialize_non_tensor_batch(non_tensor_batch: dict) -> dict:
    """Pre-serialize torch.Tensor inside non_tensor_batch for efficient pickle5."""
    result = {}
    for key, val in non_tensor_batch.items():
        if not isinstance(val, np.ndarray) or val.dtype != object or len(val) == 0:
            result[key] = val
            continue
        # Only process arrays whose elements may contain tensors (dict / list / Tensor).
        first_non_none = next((v for v in val if v is not None), None)
        if not isinstance(first_non_none, (dict, torch.Tensor, list)):
            result[key] = val
            continue
        new_arr = np.empty(len(val), dtype=object)
        for i in range(len(val)):
            new_arr[i] = _serialize_obj(val[i])
        result[key] = new_arr
    return result


def _deserialize_non_tensor_batch(non_tensor_batch: dict) -> dict:
    """Restore torch.Tensor from serialized form in non_tensor_batch."""
    result = {}
    for key, val in non_tensor_batch.items():
        if not isinstance(val, np.ndarray) or val.dtype != object or len(val) == 0:
            result[key] = val
            continue
        first_non_none = next((v for v in val if v is not None), None)
        # Serialized tensors appear as tuples or inside dicts/lists.
        if not isinstance(first_non_none, (dict, tuple, list)):
            result[key] = val
            continue
        new_arr = np.empty(len(val), dtype=object)
        for i in range(len(val)):
            new_arr[i] = _deserialize_obj(val[i])
        result[key] = new_arr
    return result
...
class DataProto:
    ...
    def __getstate__(self):
        if self.batch is not None:
            ...
            batch = None
        non_tensor_batch = _serialize_non_tensor_batch(self.non_tensor_batch)
        return (batch, non_tensor_batch, self.meta_info)
    ...
    def __setstate__(self, data):
        ...
        self.non_tensor_batch = _deserialize_non_tensor_batch(non_tensor_batch)
