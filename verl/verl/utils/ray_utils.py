...
def _estimate_obj_size(obj):
    """Recursively estimate memory footprint without any extra allocation."""
    import numpy as np
    import torch
    ...
    if isinstance(obj, np.ndarray):
        if obj.dtype != object:
            return obj.nbytes
        return sum(_estimate_obj_size(item) for item in obj)
    if isinstance(obj, dict):
        return sum(_estimate_obj_size(v) for v in obj.values())
...
def _estimate_data_size(data):
    """Estimate total byte size of data, aware of DataProto structure."""
    import numpy as np

    size = 0
    # DataProto: has batch (TensorDict) + non_tensor_batch (dict of ndarray)
    if hasattr(data, "batch") and hasattr(data, "non_tensor_batch"):
        batch = data.batch
        if batch is not None:
            for key in batch.keys(include_nested=True, leaves_only=True):
                size += _estimate_obj_size(batch[key])
        ntb = data.non_tensor_batch
        if ntb:
            for val in ntb.values():
                size += _estimate_obj_size(val)
        return size
    return _estimate_obj_size(data)
...
def parallel_put(data_list: list[Any], max_workers: Optional[int] = None):
    ...
    def put_data(index, data):
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                "ray.put[%d] %.3fs, size=%.2f MiB", index, elapsed, data_size / (1024 * 1024)
            )
            return index, ref
