    ...
    if batch is not None:
        for key, tensor in batch.items():
            if isinstance(tensor, torch.Tensor) and not tensor.is_nested:
                ...
                # Handle numpy.ndarray explicitly
                if len(tensor.shape) > 0 and tensor.shape[0] > 0:
                    ...
                    data_dict[key] = tensor[indices_np]
                else:
                    data_dict[key] = tensor
            elif isinstance(tensor, NonTensorStack):
                # NonTensorStack is indexable by batch dim
                data_dict[key] = tensor[indices]
            elif isinstance(tensor, NonTensorData):
                # Handle NonTensorData - check if data is indexable (list, array, etc.)
                indices_np = indices.cpu().numpy() if isinstance(indices, torch.Tensor) else np.array(indices)
                data = tensor.data
                if isinstance(data, list):
                    # Index list by creating a new list with selected elements
                    indexed_data = [data[i] for i in indices_np]
                    data_dict[key] = NonTensorData(indexed_data)
                elif isinstance(data, np.ndarray):
                    # Index numpy array directly
                    indexed_data = data[indices_np]
                    data_dict[key] = NonTensorData(indexed_data)
                elif isinstance(data, str):
                    # String should be treated as scalar metadata - keep as is
                    data_dict[key] = tensor
                elif hasattr(data, '__len__') and len(data) > 0:
                    # For other sequence-like types (tuple, etc.)
                    indexed_data = [data[i] for i in indices_np]
                    data_dict[key] = NonTensorData(indexed_data)
                else:
                    # Scalar metadata - keep as is
                    data_dict[key] = tensor
            else:
                # This handles other types
                # Check if tensor has a shape attribute and is indexable
