...
def get_hf_model_ref_path(config):
    if config.model.path.startswith("hdfs:"):
        from verl.utils.fs import copy_to_local

        local_model_path = copy_to_local(src=config.model.ref_path, use_shm=config.model.get("use_shm", False))
    else:
        local_model_path = config.model.ref_path
    return local_model_path
