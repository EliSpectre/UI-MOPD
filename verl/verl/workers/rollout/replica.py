...
class TokenOutput(BaseModel):
    ...
    """cache id when routed experts are materialized outside the returned payload"""
    routed_experts_cache_source: Optional[str] = None
    """cache source identifier for deferred routed experts hydration"""
...
class RolloutReplica(ABC):
    ...
    async def clear_routed_experts_cache(self):
        """Clear all routed experts cache entries on each rollout server."""
        await asyncio.gather(*[server.clear_all_routed_experts_cache.remote() for server in self.servers])
