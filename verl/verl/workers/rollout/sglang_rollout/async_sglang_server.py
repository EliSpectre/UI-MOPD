...
class SGLangHttpServer:
    ...
    async def fetch_routed_experts(self, cache_ids: list[str]) -> list[torch.Tensor]:
        missing_ids = [cache_id for cache_id in cache_ids if cache_id not in self._routed_experts_cache]
        if missing_ids:
            raise KeyError(
                f"Missing routed_experts cache entries on server={self.server_name}: "
                f"{missing_ids[:4]} (missing={len(missing_ids)})"
            )
        return [self._routed_experts_cache[cache_id] for cache_id in cache_ids]

    async def clear_all_routed_experts_cache(self) -> int:
        ...
        if count:
            logger.info(
                "clear_all_routed_experts_cache: server=%s cleared %s entries",
                self.server_name,
                count,
            )
        return count
    ...
    async def generate(
        ...
        if self.config.enable_rollout_routing_replay:
            ...
            if "return_routed_experts" in inspect.signature(GenerateReqInput.__init__).parameters:
                generate_kwargs["return_routed_experts"] = True

        request = GenerateReqInput(**generate_kwargs)
        ...
        routed_experts = None
        routed_experts_cache_id = None
        routed_experts_cache_source = None
        if self.config.enable_rollout_routing_replay:
            routed_experts = output['meta_info']["routed_experts"]
            if (
                self.rollout_mode == RolloutMode.HYBRID
                and self.config.routed_experts_server_cache
                and routed_experts is not None
            ):
                routed_experts_cache_id = uuid4().hex
                self._routed_experts_cache[routed_experts_cache_id] = torch.as_tensor(routed_experts).cpu().contiguous()
                routed_experts_cache_source = self.server_name
                routed_experts = None
        ...
        return TokenOutput(
            ...
            routed_experts_cache_id=routed_experts_cache_id,
            routed_experts_cache_source=routed_experts_cache_source,
        )
