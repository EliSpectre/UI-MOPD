...
class MegatronPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        def compute_logprobs_fn(output, data, use_dynamic_bsz=False, indices=None):
            ...
            if "student_topk_indices" in output:
                ret["student_topk_indices"] = output["student_topk_indices"][:, -response_length - 1 : -1].contiguous()
            if "teacher_topk_logprobs" in output:
                ret["teacher_topk_logprobs"] = output["teacher_topk_logprobs"][:, -response_length - 1 : -1].contiguous()
            return ret
        ...
        if recompute_old_log_prob:
            ...
            with torch.no_grad():
                ...
                extra_out = {}
                use_topk_distillation = data.meta_info.get("use_topk_distillation", False)
                if use_topk_distillation:
                    topk_size = data.meta_info.get("topk_size", 200)
                    topk_distillation_phase = data.meta_info.get("topk_distillation_phase", None)
                    if topk_distillation_phase == "student":
                        if mpu.is_pipeline_last_stage(ignore_virtual=True):
                            student_topk_indices = [o[0]["student_topk_indices"] if calculate_entropy else o["student_topk_indices"] for o in output["output"]]
                            student_topk_indices = torch.cat(student_topk_indices, dim=0)
                            if use_dynamic_bsz:
                                student_topk_indices = student_topk_indices[revert_indices]
                        else:
                            student_topk_indices = torch.empty(size=(batch_size, response_length, topk_size), dtype=torch.int64, device=input_ids.device)
                        
                        student_topk_indices = student_topk_indices.to(get_device_id())
                        torch.distributed.broadcast(
                            tensor=student_topk_indices,
                            src=mpu.get_pipeline_model_parallel_last_rank(),
                            group=mpu.get_pipeline_model_parallel_group(),
                            async_op=False,
                        )
                        extra_out["student_topk_indices"] = student_topk_indices.to("cpu")
                    elif topk_distillation_phase == "teacher":
                        if mpu.is_pipeline_last_stage(ignore_virtual=True):
                            teacher_topk_logprobs = [o[0]["teacher_topk_logprobs"] if calculate_entropy else o["teacher_topk_logprobs"] for o in output["output"]]
                            teacher_topk_logprobs = torch.cat(teacher_topk_logprobs, dim=0)
                            if use_dynamic_bsz:
                                teacher_topk_logprobs = teacher_topk_logprobs[revert_indices]
                        else:
                            teacher_topk_logprobs = torch.empty(size=(batch_size, response_length, topk_size), dtype=torch.float32, device=input_ids.device)
                        
                        teacher_topk_logprobs = teacher_topk_logprobs.to(get_device_id())
                        torch.distributed.broadcast(
                            tensor=teacher_topk_logprobs,
                            src=mpu.get_pipeline_model_parallel_last_rank(),
                            group=mpu.get_pipeline_model_parallel_group(),
                            async_op=False,
                        )
                        extra_out["teacher_topk_logprobs"] = teacher_topk_logprobs.to("cpu")
        ...
        for module, mode in zip(self.actor_module, prev_modes, strict=False):
            module.train(mode)
        return log_probs, entropys, layers_topk_idx, extra_out
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        if "student_topk_indices" in data.batch.keys():
            select_keys.append("student_topk_indices")
        if "teacher_topk_logprobs" in data.batch.keys():
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if forward_only:
                ...
                if self.config.use_kl_loss:
                    ...
                    kld = kld * rollout_is_weights if rollout_is_weights is not None else kld
                    if self.config.use_kl_loss_mask and "kl_loss_mask" in data.keys():
                        response_mask = response_mask * data["kl_loss_mask"].to(bool)                    
                    kl_loss = agg_loss(loss_mat=kld, loss_mask=response_mask, loss_agg_mode=self.config.loss_agg_mode)
                    if self.config.use_kl_loss_mask and "kl_loss_mask" in data.keys():
                        kl_loss *= data["kl_loss_mask"][:, 0].to(torch.float).mean().detach().item()
                        
                    policy_loss = policy_loss + kl_loss * self.config.kl_loss_coef
                    metrics["actor/kl_loss"] = kl_loss.detach().item()
                    metrics["actor/kl_coef"] = self.config.kl_loss_coef

                if data_meta_info.get("use_topk_distillation", False) and data_meta_info.get("topk_distillation_phase") == "update":
                    kl_divergence = output["topk_distillation_kl_divergence"][:, -response_length - 1 : -1].contiguous()
                    kl_divergence_loss = agg_loss(loss_mat=kl_divergence, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
                    topk_kl_loss_weight = data_meta_info.get("topk_kl_loss_weight", 1.0)
                    policy_loss = (1.0 - topk_kl_loss_weight) * policy_loss + topk_kl_loss_weight * kl_divergence_loss
                    metrics["actor/topk_kl_loss"] = kl_divergence_loss.detach().item()
        ...
        def forward_step(batch_iter, model, return_schedule_plan: bool = False):
            ...
            use_topk_distillation = data.meta_info.get("use_topk_distillation", False)
            if self.use_fused_kernels and not use_topk_distillation:
                ...
                def logits_processor(logits, label, label_mask, student_topk_indices=None, teacher_topk_logprobs=None):
                    ...
                    if use_topk_distillation:
                        ...
                        if topk_distillation_phase == "student":
                            from verl.utils.megatron.tensor_parallel import \
                                vocab_parallel_topk_log_probs
                            _, topk_indices = vocab_parallel_topk_log_probs(logits, topk_size)
                            ret["student_topk_indices"] = topk_indices
                        elif topk_distillation_phase == "teacher":
                            from verl.utils.megatron.tensor_parallel import \
                                vocab_parallel_topk_log_probs
                            teacher_topk_logprobs, _ = vocab_parallel_topk_log_probs(logits, topk_size, topk_indices=student_topk_indices)
                            ret["teacher_topk_logprobs"] = teacher_topk_logprobs
                        elif topk_distillation_phase == "update":
                            from verl.utils.megatron.tensor_parallel import \
                                vocab_parallel_kl_divergence
                            alpha = data.meta_info.get("alpha", 0.0)
                            kl_divergence = vocab_parallel_kl_divergence(logits.clone(), teacher_topk_logprobs, student_topk_indices, alpha=alpha)
                            ret["topk_distillation_kl_divergence"] = kl_divergence
                ...
                if use_topk_distillation:
                    ...
                    if topk_distillation_phase in ("teacher", "update") and "student_topk_indices" in batch:
                        sti = batch["student_topk_indices"]
                        full_sti = torch.zeros(sti.size(0), seq_len, sti.size(-1), dtype=sti.dtype, device=sti.device)
                        full_sti[:, -response_length - 1 : -1, :] = sti
                        logits_processor_args["student_topk_indices"] = full_sti
                    if topk_distillation_phase == "update" and "teacher_topk_logprobs" in batch:
                        ttl = batch["teacher_topk_logprobs"]
                        full_ttl = torch.zeros(ttl.size(0), seq_len, ttl.size(-1), dtype=ttl.dtype, device=ttl.device)
                        full_ttl[:, -response_length - 1 : -1, :] = ttl
                        logits_processor_args["teacher_topk_logprobs"] = full_ttl
                output = forward_fn(
                    model=model,
                    input_ids=input_ids,
                    attention_mask=attention_mask,
