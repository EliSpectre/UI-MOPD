...
from ui_mopd.dataset.dynsam_dataloader import DynamicSamplingDataloader
...
def _build_action_only_mask(responses: torch.Tensor, response_mask: torch.Tensor, think_end_token_id: int = 151668):
    """Build a mask that only covers tokens after </think>, excluding thinking tokens.

    Args:
        responses: response token ids [batch_size, seq_len]
        response_mask: original response mask [batch_size, seq_len]
        think_end_token_id: token id for </think> (default 151668 for Qwen3)

    Returns:
        action_mask: mask with 1 only for tokens after </think>
    """
    action_mask = torch.zeros_like(response_mask)
    for i in range(responses.shape[0]):
        think_end_positions = (responses[i] == think_end_token_id).nonzero(as_tuple=True)[0]
        if len(think_end_positions) > 0:
            action_start = think_end_positions[0].item() + 1
            action_mask[i, action_start:] = response_mask[i, action_start:]
        else:
            action_mask[i] = response_mask[i]
    return action_mask


def apply_kl_penalty_advantage(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty="kl", kl_skip_thought=False):
    """Apply KL penalty to the token-level rewards.
    ...
    Args:
        ...
        kl_ctrl (core_algos.AdaptiveKLController): Controller for adaptive KL penalty.
        kl_penalty (str, optional): Type of KL penalty to apply. Defaults to "kl".
        kl_skip_thought (bool, optional): If True, only compute KL on action tokens (after </think>).
    ...
    # If kl_skip_thought, only compute KL on action tokens (after </think>)
    if kl_skip_thought and "responses" in data.batch:
        action_mask = _build_action_only_mask(data.batch["responses"], response_mask)
        kld = kld * action_mask
    else:
        kld = kld * response_mask
    beta = kl_ctrl.value

    kld = kld * data.batch["kl_loss_mask"]
    advantages = advantages * data.batch["passrate_0_mask"]
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.use_kl_in_reward or self.config.algorithm.use_kl_in_advantage:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        if not self.config.data.dynamic_sampling.enabled:
            ...
            self.train_dataloader = DynamicSamplingDataloader(
                dataset=self.train_dataset,
                total_step=self.config.data.dynamic_sampling.total_step,
                collate_fn=collate_fn,
                passrate1_strategy=self.config.data.dynamic_sampling.passrate1_strategy,
                shuffle=self.config.data.shuffle,
                replay_decay_factor=self.config.data.dynamic_sampling.replay_decay_factor,
                replay_min_ratio=self.config.data.dynamic_sampling.replay_min_ratio,
                reset_queue=self.config.data.dynamic_sampling.reset_queue,
            )
    ...
    def _drop_routed_experts_from_batch(self, batch: DataProto):
        if batch.batch is None or ROUTED_EXPERTS_BATCH_KEY not in batch.batch.keys():
            return
        batch.batch.pop(ROUTED_EXPERTS_BATCH_KEY)

    def _clear_routed_experts_caches(self, batch: DataProto = None):
        """Clear all R3 routed experts caches on actor workers and sglang servers."""
        rollout_cfg = self.config.actor_rollout_ref.rollout
        if not bool(rollout_cfg.get("routed_experts_server_cache", False)):
            return

        # Clear actor worker-side cache
        if self.use_legacy_worker_impl != "disable":
            clear_method = getattr(self.actor_rollout_wg, "clear_all_routed_experts_cache", None)
            if clear_method is not None:
                clear_method()

        # Clear sglang server-side cache
        if hasattr(self, "async_rollout_manager") and self.async_rollout_manager is not None:
            clear_method = getattr(self.async_rollout_manager, "clear_all_routed_experts_caches", None)
            if clear_method is not None:
                clear_method()

        # Clean up batch keys
        if batch is not None:
            batch.non_tensor_batch.pop(ROUTED_EXPERTS_CACHE_SOURCE_KEY, None)
            batch.non_tensor_batch.pop(ROUTED_EXPERTS_CACHE_ID_KEY, None)
